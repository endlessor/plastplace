...
class EditListing extends Component {
  ...
  handleRemoveUrl = urlToDelete => {
    ...
    newImageUrls = this.state.imageUrls.filter(
      (item, index) => item.id !== urlToDelete
    );
  ...
  handleClickDone = async () => {
    const { images, imageUrls, material, availability, price, location } = this.state;
  ...
  renderTitle = () => (
    <div className="container text-center">
      <div className="create-listing-title p-5">
        <h3><Translate value="create_listing.edit_listing" /></h3>
      </div>
    </div>
  );
